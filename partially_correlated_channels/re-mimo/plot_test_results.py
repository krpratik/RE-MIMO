...
with open('./final_results'+'/network_corr_validtn_results_new.pickle', 'rb') as handle:
	network_validtn_results_dict = pickle.load(handle)

with open('./final_results' + '/oampnet_corr_16_validtn_results.pickle', 'rb') as handle:
	oampnet_16_validtn_results_dict = pickle.load(handle)

with open('./final_results' + '/oampnet_corr_32_validtn_results.pickle', 'rb') as handle:
...
corr_list = np.asarray([0.70, 0.80])
markers_list= ['o','v']
...
for NT in NT_list:
	...
	for i, rho in enumerate(corr_list):
		...
		plt.plot(network_result_keys[initial_point+1: initial_point+7], network_result_values[initial_point+1: initial_point+7], label='RE-MIMO '+rho_label, marker=markers_list[i], ms=7, color='r', fillstyle='none')
		...
		plt.plot(oampnet_result_keys[initial_point: initial_point+6], oampnet_result_values[initial_point: initial_point+6], label='OAMPNet '+rho_label , marker=markers_list[i], ms=6, color='k', fillstyle='none', ls=':')
		...
		plt.tight_layout()
		plt.savefig('./final_results/' + '/' + mod + '_NT' + str(NT) + '_NR64_corr.pdf')
plt.show()