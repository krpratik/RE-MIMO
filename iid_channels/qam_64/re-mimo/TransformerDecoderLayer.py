...
class TransformerDecoderLayer(nn.Module):
	...
	def forward(self, st, common_input, noise_sigma, NT):
		...
		del out
		out_real = out_real.unsqueeze(dim=-1)
		out_imag = out_imag.unsqueeze(dim=-2)
		out = (out_real + out_imag).view(NT, -1, self.mod_n)
		del out_imag, out_real
		return out