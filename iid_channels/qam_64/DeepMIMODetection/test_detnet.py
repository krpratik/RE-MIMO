...
detnet_validtn_filename = './final_results/detnet_' + str(NT) + '_validtn_results.pickle'
...
def validate_model_given_data(model, validtn_H, validtn_y, validtn_j_indices, real_QAM_const, imag_QAM_const, device):
	...
	with torch.no_grad():
		...
		list_batch_x_predicted = model.forward(validtn_batch_size, HTY, HTH)
		validtn_out = list_batch_x_predicted[-1].to(device='cpu')
		indices_detnet = sym_detection(validtn_out, validtn_j_indices, real_QAM_const, imag_QAM_const)
		accr = sym_accuracy(indices_detnet, validtn_j_indices)
...
def validate_detnet(model, generator, device, real_QAM_const, imag_QAM_const, save_result=True):
	...
	for iter in range(validtn_iter):
		...
		for NT in validtn_NT_list:
			for snr in snrdb_classical_list[NT]:
				...
				accr = validate_model_given_data(model, big_validtn_H, big_validtn_y, big_validtn_j_indices, real_QAM_const, imag_QAM_const, device)
				result_dict[NT][snr] =  result_dict[NT][snr] + (accr-result_dict[NT][snr])/float(iter+1.0)

		if (save_result):
			with open(detnet_validtn_filename, 'wb') as handle:
				pickle.dump(result_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)
			print('Intermediate Test results saved at : ', detnet_validtn_filename)
...
def test(model, generator, device):
	...
	validate_detnet(model, generator, device, generator.real_QAM_const, generator.imag_QAM_const, True)
