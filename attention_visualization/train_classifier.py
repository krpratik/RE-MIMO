...
validtn_NT_list = np.asarray([12])
snrdb_list = {12:np.arange(10, 18)}
factor_list = (validtn_NT_list/validtn_NT_list.sum())/snrdb_list[12].size
...
def mini_validation(model, mini_validation_dict, i, device, criterion=None, save_to_file=True):
	...
	print('Validation result : ', result_dict)
	if (save_to_file):
...
def save_model_func(model, optimizer):
	...
	print('******Model Saved********** at durectory : ', model_filename)
...
def main():
	generator = sample_generator(train_batch_size, mod_n, NR, NT_list[0])
	device = 'cuda'
	...
	if (load_pretrained_model):
		...
		lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer,'min', 0.90, 0, True, 0.0001, 'rel', 0, 0, 1e-08)
